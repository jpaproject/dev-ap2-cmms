...
<body>
    <div class="content">
        <div class="header">
            <div class="title">
                <table style="padding:20px 20px 0px 20px;" class="title-table">
                    ...
                    <tr style="background-color: white;">
                        <td>Control desk, Pc Control Monitoring & Panel Automation</td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="body">
            <div class="table" class="my-table">
                <table>
                    <tr>
                        <th style="width: 5%" rowspan="2">NO.</th>
                        <th style="width: 20%" rowspan="2">NAMA PERALATAN</th>
                        <th style="width: 40%" colspan="2">Genset</th>
                    ...
                    <tr class="row-title">
                        <td style="min-width: 3%">I</td>
                        <td>CONTROL</td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                    @foreach ($conrtolDesks as $key => $conrtolDesk)
                        <tr>
                            <td>{{ $loop->iteration }}.</td>
                            <td>{{ $conrtolDesk->nama_peralatan }}</td>
                            <td>{{ $conrtolDesk->gs1 }}</td>
                            <td>{{ $conrtolDesk->gs2 }}</td>
                            <td>{{ $conrtolDesk->keterangan }}</td>
                        </tr>
                    @endforeach
                    <tr class="row-title">
                        <th>II</th>
                    ...
                    @foreach ($gensets as $key => $genset)
                        <tr>
                            <td>{{ $loop->iteration }}.</td>
                            <td>{{ $genset->nama_peralatan }}</td>
                            <td>{{ $genset->gs1 }}</td>
                            <td>{{ $genset->gs2 }}</td>
                            <td>{{ $genset->keterangan }}</td>
                        </tr>
                    @endforeach
                    <tr class="row-title">
                        <th>III</th>
                        <th colspan="3">GENSET</th>
                        <th>KAPASITAS LITER</th>
                    </tr>
                    @foreach ($groundTanks as $key => $groundTank)
                        <tr>
                            <td>{{ $loop->iteration }}.</td>
                            <td colspan="3">{{ $groundTank->nama_peralatan }}</td>
                            <td>{{ $groundTank->keterangan }}</td>
                        </tr>
                    @endforeach
                </table>
            </div>
        ...
        <div class="catatan" width="100">
            ...
            {!! nl2br($formPs1ControlDeskTahunans[0]->catatan ?? '') !!}
        </div>
        {{-- <div class="ttd-left">
