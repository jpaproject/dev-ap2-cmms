...
@section('content')
    <section class="content">
        ...
        <form method="POST" action="{{ route('form.apm.elektrikal-vehicle-exterior-esk-tiga-bulanan.update', $detailWorkOrderForm) }}"
                            ...
                            <div class="card-header" id="Seat1">
                                <h2 class="mb-0">
                                    <button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse"
                                        ...
                                        Extention System Contactor (ESK)
                                    </button>
                                </h2>
                            </div>
                            ...
                            <div id="collapseSeat1" class="show collapse" aria-labelledby="1" data-parent="#accordionSeat1">
                                <div class="card-body">
                                    ...
                                    @foreach ($formApmElektrikalVehicleExteriorESKTigaBulanans as $key => $formApmElektrikalVehicleExteriorESKTigaBulanan)
                                        @if ($formApmElektrikalVehicleExteriorESKTigaBulanan->pemeriksaan_esk_group == 'Extention System Contactor (ESK)')
                                            <div class="row">
                                                <div
                                                    class="col-12 col-lg-3 d-flex justify-content-center align-items-center">
                                                    <h5>{{ $loop->iteration }}
                                                        {{ $formApmElektrikalVehicleExteriorESKTigaBulanan->pemeriksaan_esk }}</h5>
                                                </div>
                                                        ...
                                                        <div class="col-12">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorESKTigaBulanan->hasil_mc1 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorESKTigaBulanan->hasil_mc1 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                        ...
                                                        {{-- <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorESKTigaBulanan->hasil_mc2 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorESKTigaBulanan->hasil_mc2 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="d-flex justify-content-center">Referensi</label>
                                                            <textarea name="referensi[]" class="form-control @error('referensi.'.$key) is-invalid @enderror"
                                                                id="referensi" placeholder="Referensi">{!! $formApmElektrikalVehicleExteriorESKTigaBulanan->referensi !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
