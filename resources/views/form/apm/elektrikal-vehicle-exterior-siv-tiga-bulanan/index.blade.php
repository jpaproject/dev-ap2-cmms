...
@section('content')
    <section class="content">
        ...
        <form method="POST" action="{{ route('form.apm.elektrikal-vehicle-exterior-siv-tiga-bulanan.update', $detailWorkOrderForm) }}"
                            ...
                            <div id="collapseSeat1" class="show collapse"  aria-labelledby="1" data-parent="#accordionSeat1">
                                <div class="card-body">
                                    ...
                                    @foreach ($formApmElektrikalVehicleExteriorSIVTigaBulanans as $key => $formApmElektrikalVehicleExteriorSIVTigaBulanan)
                                        @if ($formApmElektrikalVehicleExteriorSIVTigaBulanan->pemeriksaan_siv_group == 'STATIC INVERTER (SIV)')
                                            <div class="row">
                                                <div
                                                    class="col-12 col-lg-3 d-flex justify-content-center align-items-center">
                                                    <h5>{{ $loop->iteration }}
                                                        {{ $formApmElektrikalVehicleExteriorSIVTigaBulanan->pemeriksaan_siv }}</h5>
                                                </div>
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">SIV
                                                                    ...
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorSIVTigaBulanan->hasil_mc1 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorSIVTigaBulanan->hasil_mc1 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">SIV
                                                                    ...
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorSIVTigaBulanan->hasil_mc2 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorSIVTigaBulanan->hasil_mc2 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="d-flex justify-content-center">Referensi</label>
                                                            <textarea name="referensi[]" class="form-control @error('referensi.'.$key) is-invalid @enderror"
                                                                id="referensi" placeholder="Referensi">{!! $formApmElektrikalVehicleExteriorSIVTigaBulanan->referensi !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
