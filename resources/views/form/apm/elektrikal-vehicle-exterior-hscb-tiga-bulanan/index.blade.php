...
@section('content')
    <section class="content">
        ...
        <form method="POST" action="{{ route('form.apm.elektrikal-vehicle-exterior-hscb-tiga-bulanan.update', $detailWorkOrderForm) }}"
                            ...
                            <div id="collapseSeat1" class="show collapse" aria-labelledby="1" data-parent="#accordionSeat1">
                                <div class="card-body">
                                    ...
                                    @foreach ($formApmElektrikalVehicleExteriorHSCBTigaBulanans as $key => $formApmElektrikalVehicleExteriorHSCBTigaBulanan)
                                        @if ($formApmElektrikalVehicleExteriorHSCBTigaBulanan->pemeriksaan_hscb_group == 'HIGH SPEED CIRCUIT BRAKE (HSCB)')
                                            <div class="row">
                                                <div
                                                    class="col-12 col-lg-3 d-flex justify-content-center align-items-center">
                                                    <h5>{{ $loop->iteration }}
                                                        {{ $formApmElektrikalVehicleExteriorHSCBTigaBulanan->pemeriksaan_hscb }}</h5>
                                                </div>
                                                        ...
                                                        <div class="col-12">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorHSCBTigaBulanan->hasil_mc1 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorHSCBTigaBulanan->hasil_mc1 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                        ...
                                                        {{-- <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorHSCBTigaBulanan->hasil_mc2 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorHSCBTigaBulanan->hasil_mc2 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="d-flex justify-content-center">Referensi</label>
                                                            <textarea name="referensi[]" class="form-control @error('referensi.'.$key) is-invalid @enderror"
                                                                id="referensi" placeholder="Referensi">{!! $formApmElektrikalVehicleExteriorHSCBTigaBulanan->referensi !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
