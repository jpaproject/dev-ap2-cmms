...
@section('content')
    <section class="content">
        ...
        <form method="POST" action="{{ route('form.apm.elektrikal-vehicle-exterior-ljb-tiga-bulanan.update', $detailWorkOrderForm) }}"
                            ...
                            <div id="collapseSeat1" class="show collapse"  aria-labelledby="1" data-parent="#accordionSeat1">
                                <div class="card-body">
                                    ...
                                    @foreach ($formApmElektrikalVehicleExteriorLJBTigaBulanans as $key => $formApmElektrikalVehicleExteriorLJBTigaBulanan)
                                        @if ($formApmElektrikalVehicleExteriorLJBTigaBulanan->pemeriksaan_ljb_group == 'LOW TENSION JUNCTION BOX (LJB)')
                                            <div class="row">
                                                <div
                                                    class="col-12 col-lg-3 d-flex justify-content-center align-items-center">
                                                    <h5>{{ $loop->iteration }}
                                                        {{ $formApmElektrikalVehicleExteriorLJBTigaBulanan->pemeriksaan_ljb }}</h5>
                                                </div>
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">LJB MC
                                                                    1</label>
                                                                    <select class="form-control  @error('hasil_mc1.' . $key) is-invalid @enderror"
                                                                    name="hasil_mc1[]">
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorLJBTigaBulanan->hasil_mc1 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorLJBTigaBulanan->hasil_mc1 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">LJB MC
                                                                    ...
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorLJBTigaBulanan->hasil_mc2 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorLJBTigaBulanan->hasil_mc2 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="d-flex justify-content-center">Referensi</label>
                                                            <textarea name="referensi[]" class="form-control @error('referensi.'.$key) is-invalid @enderror"
                                                                id="referensi" placeholder="Referensi">{!! $formApmElektrikalVehicleExteriorLJBTigaBulanan->referensi !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
