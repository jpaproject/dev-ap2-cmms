...
@section('content')
    <section class="content">
        ...
        <form method="POST" action="{{ route('form.apm.elektrikal-vehicle-exterior-bec-tiga-bulanan.update', $detailWorkOrderForm) }}"
                            ...
                            <div id="collapseSeat1" class="show collapse" aria-labelledby="1" data-parent="#accordionSeat1">
                                <div class="card-body">
                                    ...
                                    @foreach ($formApmElektrikalVehicleExteriorBECTigaBulanans as $key => $formApmElektrikalVehicleExteriorBECTigaBulanan)
                                        @if ($formApmElektrikalVehicleExteriorBECTigaBulanan->pemeriksaan_brake_group == 'BRAKE ELECTRONIC CONTROL UNIT')
                                            <div class="row">
                                                <div
                                                    class="col-12 col-lg-3 d-flex justify-content-center align-items-center">
                                                    <h5>{{ $loop->iteration }}
                                                        {{ $formApmElektrikalVehicleExteriorBECTigaBulanan->pemeriksaan_brake }}</h5>
                                                </div>
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorBECTigaBulanan->hasil_mc1 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc1.' . $key) ?? $formApmElektrikalVehicleExteriorBECTigaBulanan->hasil_mc1 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                        ...
                                                        <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="d-flex justify-content-center">MC
                                                                    ...
                                                                    <option selected value="">Choose Selection</option>
                                                                    <option value="ok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorBECTigaBulanan->hasil_mc2 == 'ok' ? 'selected' : '' }}>
                                                                        Ok</option>
                                                                    <option value="nok"
                                                                        {{ old('hasil_mc2.' . $key) ?? $formApmElektrikalVehicleExteriorBECTigaBulanan->hasil_mc2 == 'nok' ? 'selected' : '' }}>
                                                                        Nok</option>
                                                                </select>
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="d-flex justify-content-center">Referensi</label>
                                                            <textarea name="referensi[]" class="form-control @error('referensi.'.$key) is-invalid @enderror"
                                                                id="referensi" placeholder="Referensi">{!! $formApmElektrikalVehicleExteriorBECTigaBulanan->referensi !!}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
